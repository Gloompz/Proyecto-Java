...
public class impldistrito implements Idistrito{
    ...
    public void findAllCustom(JTable jtbdistrito) {
        ...
        try {
            xcon = objconexion.Conectar();
            st = xcon.createStatement();
            rs = st.executeQuery("SELECT * FROM distrito WHERE estdis=1");
            while (rs.next()) {
                fila[0] = rs.getInt("coddis");
                fila[1] = rs.getString("nomdis");
                fila[2] = "Habilitado";
    ...
    public int GenerarCodigo() {
        ...
        try {
            xcon = objconexion.Conectar();
            st = xcon.createStatement();
            rs = st.executeQuery("SELECT MAX(coddis) FROM distrito");
            if (rs.next()) {
                cod = rs.getInt(1) + 1;
            } else {
                cod = 1;
    ...
    public boolean add(distrito obj) {
        ...
        try {
            xcon = objconexion.Conectar();
            pst = xcon.prepareStatement("INSERT INTO distrito(coddis, nomdis, estdis) VALUES(?,?,?)");
    ...
    public boolean update(distrito obj) {
        ...
        try {
            xcon = objconexion.Conectar();
            pst = xcon.prepareStatement("UPDATE distrito SET nomdis=?, estdis=? WHERE coddis=?");
    ...
    public boolean delete(distrito obj) {
        ...
        try {
            xcon = objconexion.Conectar();
            pst = xcon.prepareStatement("UPDATE distrito SET estdis=0 WHERE coddis=?");
            pst.setInt(1, obj.getCodigo());
            int r = pst.executeUpdate();
            if (r > 0) {
                res = true;
    ...
    public void CargarCombo(JComboBox cbodistrito) {
        ...
        try {
            ...
            while (rs.next()) {
                dis = rs.getString("nomdis");
                cbodistrito.addItem(dis);
            }
        } catch (SQLException ex) {
            System.out.println("Error: " + ex.toString());
        }
